...
public class ZonaClient {
	...
	/**
	 * lista zona para autoridade por matrícula.
	 * 
	 * @param matricula
	 * @return lista de zona para autoridade
	 */
	public List<Integer> listarParaAutoridadePorMatricula(String matricula) {
		if (StringUtil.isPreenchida(FacesUtil.getSessaoBean().getParametro().getUrlEntidadeTre())) {
			...
			if (response.getStatus() == RespostaEnum.OK.getId()) {
				...
				return new Gson().fromJson(ja, new TypeToken<List<Integer>>() {}.getType());
			// token expirado.
